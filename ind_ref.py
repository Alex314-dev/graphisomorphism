...
def get_f_from_alpha(alpha):
    f = []
    # TODO: get f based on alphal
    return f


def generate_automorphism(D, I, U, y_to_its_false_twins):
    alpha = color_refinement([], [], U)

    if not is_balanced(alpha):
        return
    if is_bijection(alpha):
        f = get_f_from_alpha(alpha)
        membership = membership_testing(f)
        if not membership:
            # X = X + [f] #TODO: resolve the problem (something related to X being global)
            return

    # UPDATE THE FOLLOWING PART (SHOULD BE ADJUSTED TO THE CURRENT ALGORITHM)
    ...
    for y in alpha[1][color_class]:  # vertices in H with color `color_class`
        ...
        # TODO: If an isomorphism has been found, translate it into a permutation, and add it to the generating set (X?)
        num = num + temp_sol


def find_generating_set(G):
    U = union(G, G)
    y_to_its_false_twins = build_false_twins(U)

    generate_automorphism([], [], U, y_to_its_false_twins)

    return X


def membership_testing(q):  #
    pass  # TODO: is q generated by the (global) set X?
